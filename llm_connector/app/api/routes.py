...
async def query_llm(
    request: QueryRequest, gemini_service: GeminiService = Depends(get_gemini_service)
):
    logger.info(
    ...
    try:
        response_text = gemini_service.generate_response(
            prompt=request.prompt, context=request.context, model=request.model
        )

        logger.info("Query processed successfully")
        return QueryResponse(
            response=response_text, prompt=request.prompt, model_used=request.model
        )
