...
class CNNInterface:
    ...
    def clasificar_imagen(
        ...
        imagen,
        filtro: str
    ) -> Tuple[Optional[np.ndarray], str, pd.DataFrame]:
        """Clasifica imagen y devuelve imagen procesada, resultado Markdown y DataFrame para BarPlot."""
        # DataFrame vacío válido para BarPlot
        empty_df = pd.DataFrame({"clase": [], "probabilidad": []})

        if imagen is None:
            return None, "Por favor, carga una imagen.", empty_df
        ...
        try:
            ...
            # Clasificar con el microservicio CNN
            ...
            probabilidades = resultado.get("probabilities", {}) or {}
...
"""
            ...
            # Preparar DataFrame para BarPlot (columnas deben coincidir con x="clase", y="probabilidad")
            # Ordenamos por clase numérica para que se vea 0..9
            clases = []
            probs = []
            ...
            # Si son clases numéricas, ordenarlas
            if len(df_probs) > 0 and pd.api.types.is_numeric_dtype(df_probs["clase"]):
                df_probs = df_probs.sort_values("clase")

            return img_array, mensaje, df_probs

        except Exception as error:
            logger.error(f"Error al clasificar imagen: {error}", exc_info=True)
            return None, f"Error: {str(error)}", empty_df
